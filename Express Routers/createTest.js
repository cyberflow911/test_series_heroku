...
{
    var negative =0;
    const {name, description,negativeMarking,correctAnswerMarking,totalMarks,duration, selectedDate, selectedTime, testType, minimumMarks, language, price} = req.body;
    const data = {name, description, negativeMarking, correctAnswerMarking, totalMarks, selectedDate, selectedTime, testType};
    if(negativeMarking)
    {
    ...
    const resultFromJoi = testValidator('name description negativeMarking correctAnswerMarking totalMarks selectedDate selectedTime testType minimumMarks', data);
    console.log(data);
...
router.get('/getTestBySubCategory/:subCategoryID/:offset/:limit', async(req, res)=>

{
    const limit = parseInt(req.params.limit);
    const offset = (parseInt(req.params.offset)-1)* limit
    try {
        const test = await Test.find({subCategoryID: req.params.subCategoryID}, {}, {sort:
        {
            'createdAt': -1
...
router.get('/getQuestions/:testID/:offset/:limit', async (req, res)=>
{
    ...
    try {
        ...
        const question = await Question.find({testID: req.params.testID}, {}, {sort:
        {
            'createdAt': 1
        }}).limit(limit).skip(offset);
...
router.get('/getTest/:testID/:language', async (req,res)=>
{
    if(req.params.language === 'English')
    {
        try {
            ...
            const test = await Test.findOne({_id: req.params.testID}).populate('mainContentEnglish');
    ...
    else 
    {
        try {
            ...
            const test = await Test.findOne({_id: req.params.testID}).populate('mainContentHindi')
...
router.get('/getTest/:testID/:language/:limitQuestion/:offsetQuestion', async (req,res)=>
{
    ...
    if(req.params.language === 'English')
    {
        try {
            ...
            const test = await Test.findOne({_id: req.params.testID}).populate([{path: 'mainContentEnglish',options: 
        {
            limit: limitQuestion
    ...
    else 
    {
        try {
            ...
            const test = await Test.findOne({_id: req.params.testID}).populate([{path: 'mainContentHindi',options: 
            {
                skip: offset,
                limit: limitQuestion,
...
router.post('/testArrayTextOptions',upload.fields([
    {
        name: "questionImage"
    }
]), async(req, res)=>
        ...
        if(req.body[i].optionType === "Text")
        {
                ...
                else 
                {
                    ...
                    req.body[i].options.forEach(async( optionValue) => {
                        const updateQuestion = await Question.updateMany({_id: newQuestion._id}, 
                            {
                                $push:
        ...
        for(let i=0; i< req.body.length ; i++)
        {
            ...
            if(req.body[i].optionType === "Text")
            {
                    ...
                    else 
                    {
                        ...
                        const user = await Test.updateMany({_id: req.params.testID}, 
                            {
                                $push:
                                {
...
router.post('/testArray',upload.fields([
    {
        name: 'questionImage', maxCount: 5
    },
    {
        name: 'optionsImage'
    }
...
router.post('/postQuestion/:testID',upload.fields([
    {
        ...
        else 
        {
            ...
            else 
            {
                ...
                else 
                {
                    ...
                    const updateOption = await Question.updateMany({_id: addQuestion._id}, 
                        {
                            $push:
                            {
                                options: data
...
router.post('/addQuestion',upload.fields([
    {
        name: 'questionImage', maxCount: 1
...
router.post('/addQuestion/:question',upload.fields([
    {
        name: 'questionImage', maxCount: 1
...

{
    ...
    //checking testID 
    try {
        const questionImage = await Question.findOne({_id: req.params.questionID});
        if(!questionImage)
        {
            res.status(200).json(
                {
...
router.post('/addQuestionTest',upload.fields([
    {
        name: "optionsImage", maxCount: 9
    }
]), async(req,res)=>
        ...
        try {
            ...
            else 
            {
                ...
                req.files.optionsImage.forEach(async (element) => {
                    ...
                    const updateOptions = await Question.updateMany({_id: questionImage._id},
                        {
                            $push:
                            {
                                options: data
...
router.put('/editQuestion/:questionID', async(req,res)=>
{
    try {
        ...
        {
            const updateQuestion = await Question.findOneAndUpdate({_id: req.params.questionID}, 
                req.body);
...
router.put('/editQuestionData/:questionID',async(req, res)=>
{
    ...
    try {
        ...
        {
            const questionUpdate = await Question.updateMany({_id: req.params.questionID},
                {
                    answer: answer,
                    question: question,
...
router.put('/editOption/:questionID/:optionsID',upload.single('optionsImage'), async(req, res)=>

{
    const {option, optionType} = req.body;
    try {
        console.log(req.file);
        const findQuestion = await Question.findOne({_id: req.params.questionID}, 
            {
                options:
                {
                    $elemMatch:
...
router.post('/createTest/:subCategoryID/:mainTestID/:language', async(req, res)=>
{
    var negative =0;
    const {name, description,negativeMarking,correctAnswerMarking,totalMarks,duration, selectedDate, selectedTime, testType, minimumMarks,QuestionCount} = req.body;
    const data = {name, description, negativeMarking, correctAnswerMarking, totalMarks, selectedDate, selectedTime, testType};
    if(negativeMarking)
    {
        negative = negativeMarking;
    ...
    else 
    {
        ...
        try {
            ...
            if(!test)
            {
                ...
                await test.save();
                try {
                    const testUpdate = await Test.updateOne({_id: req.params.mainTestID}, 
                        {
...
router.post('/addQuestionRealTest/:testID',upload.fields([
    {
        name: "optionsImage", maxCount: 9
    }
]), async(req,res)=>
...
router.post('/addQuestionTest/:questionID',upload.fields([
    {
        name: 'optionsImage', maxCount: 9
    }
]), async(req,res)=>
{
    ...
    else
    {
        if(addQuestion.optionFormat === 'File')
        {
            ...
            try {
                ...
                const setArray = await Question.updateMany({_id: req.params.questionID}, 
                    {
                        $set:
                        {
                            options: []
    ...
    else 
    {
        ...
        {
            try {
                const questionImage = await Question.updateMany({_id: req.params.questionID},req.body);
                if(!questionImage)
                {
                    res.status(
                        {
                ...
                else 
                {
                    ...
                    req.files.optionsImage.forEach(async (element) => {
                        ...
                        console.log(data);
                        const updateOptions = await Question.updateMany({_id: req.params.questionID},
                            {
                                $push:
                                {
                                    options: data
        ...
        {
            try {
                const addQuestion = await Question.updateMany({_id: req.params.questionID},req.body);
                if(!addQuestion)
                {
                    res.status(200).json(
                        {
...
router.post('/addOptions/:questionID',upload.fields([
    {
    ...
    try {
        ...
        else 
        {
            ...
            try {
                ...
                const addOption = await Question.updateMany({_id: req.params.questionID}, 
                    {
                        $push:
                        {
                            options: data
...
router.delete('/deleteQuestion/:testID/:questionID/:language', async(req, res)=>
{
    try {
        const test = await Test.findOne({_id: req.params.testID});
        console.log(test);
            ...
            else 
            {
                ...
                if(req.params.language === 'English')
                {
                    try {
                        ...
                        const deleteQuestion = await Test.updateMany({_id: req.params.testID}, {
                            $pull:
                            {
                                mainContentEnglish: {
...
router.get('/getAllTests', async(req,res)=>
{
    try {
        ...
        const test = await Test.find({},{},{sort:
            {
                'createdAt': -1
            }});
        if(!test)
...
router.get('/getCategoriesSub/:offset/:limit', async(req, res)=>
{
    ...
    try {
        const result = await Category.find().populate('subCategory').limit(limit).skip(offset);
...
router.delete('/deleteTest/:testID', async( req, res)=>
{
    try {
        ...
        {
            try {
                const deleteTest = await Test.findOneAndDelete({_id: req.params.testID});
                if(!deleteTest)
                {
                    res.status(200).json(
                        {
...
router.post('/editTest/:testID', async(req, res)=>
{
    ...
    else 
    {
        ...
        var {name, description,negativeMarking,correctAnswerMarking,totalMarks,duration, selectedDate, selectedTime, testType, minimumMarks} = req.body;
        var data = {name, description, negativeMarking, correctAnswerMarking, totalMarks, selectedDate, selectedTime, testType};

        var resultFromJoi = testValidator('name description negativeMarking correctAnswerMarking totalMarks selectedDate selectedTime testType minimumMarks', data);
        ...
        else 
        {
            try {
                ...
                const test = await Test.updateMany({_id: req.params.testID},{
                    name: name,
                    description: description,
                    subCategoryID: req.params.subCategoryID,
...
router.put('/importQUestion/:testID/:questionID', async(req, res)=>
{
    try {
        ...
        {
            const questionUpdate = await Test.findOneAndUpdate({_id: req.params.testID}, 
                {
                    $push:
                    {
