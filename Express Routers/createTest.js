...
router.get('/getTest/:testID', async (req,res)=>
{
    try {
        ...
        const test = await Test.findOne({_id: req.params.testID}).populate('mainContent');
    ...
    try {
        ...
        const findQuestion = await Question.findOne({_id: req.params.questionID}, 
            ...
            else 
            {
                if(optionType === 'File')
                {
                    console.log("requesting file ",req);
                    if(!req.file)
                    {
                    ...
                    else 
                    {
                        ...
                        const updateOption = await Question.updateMany(
                            ...
                            {
                                $set: {
                                    "options.$.option": req.file.filename,
                                    "options.$.optionType": "File",
                                 }
                            }
                        )
                ...
                if(question.optionFormat === 'File')
                {
                    ...
                    try {
                        ...
                        const deleteQuestion = await Test.updateMany({_id: req.params.testID}, {
                            $pull:
                            {
                                mainContent: req.params.questionID
...
router.post('/editTest/:testID', async(req, res)=>
{
    ...
    else 
    {
        ...
        var {name, description,negativeMarking,correctAnswerMarking,totalMarks,duration, selectedDate, selectedTime, testType, minimumMarks,QuestionCount} = req.body;
